...
def get_files(number_of_round):
	...
	#for coloured input
		...
		allX = np.zeros((settings.number_of_files//settings.memory_limit + 1, settings.picture_input_dimension, settings.picture_input_dimension, settings.image_depth), dtype='float64')
	...
	for memrange in range( (number_of_round - 1 ) * number_of_limit , number_of_round * (number_of_limit) + 1):
		...
		try:
			...
			resized_img = np.resize(img, (settings.picture_input_dimension, settings.picture_input_dimension ))
